    ...
    if (block != LX_NAND_BLOCK_UNMAPPED)
    {
        ...
        if (block_status & LX_NAND_BLOCK_STATUS_NON_SEQUENTIAL)
        {
            ...
            for (page = (LONG)available_pages - 1; page >= 0; page--)
            {
                ...
                {
#ifdef LX_NAND_ENABLE_CONTROL_BLOCK_FOR_DRIVER_INTERFACE
                    status = (nand_flash -> lx_nand_flash_driver_pages_read)(nand_flash, block, (ULONG)page, (UCHAR*)buffer, LX_NULL, 1);
#else
                    status = (nand_flash -> lx_nand_flash_driver_pages_read)(block, (ULONG)page, (UCHAR*)buffer, LX_NULL, 1);
#endif
                    ...
                    return (status) ? (LX_ERROR) : (LX_SUCCESS);
                }
            }
        }
        else
